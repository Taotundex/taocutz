...
const page = () => {
  return (
    <div className='h-screen bg-black text-white w-full flex flex-col items-center overflow-y-hidden md:bg-none bg-[url(/images/signup.jpeg)] bg-cover bg-right-top bg-no-repeat'>
            ...
            <div className="myform h-auto flex items-center md:w-full w-[95%] md:px-0 px-[30px] py-[30px] md:bg-transparent bg-[#00000080] md:border-none border-[2px] border-[#ffffff49] backdrop-blur-[25px] rounded-[20px] mx-auto">
                <form action="" className='flex flex-col gap-[10px] w-full'>
                    ...
                    <center className='flex flex-col pb-5'>
                        <span className='text-[40px] font-extrabold'>SIGNUP</span>
                        <span className='text-sm text-[#ffffffbe]'>Are you an existing user? <Link href="login" className='underline text-[#ffffff80] underline-offset-4'>Kindly login here</Link></span>
                    </center>
                    <div className="flex flex-col gap-[20px]">
                        <div className="inputs flex flex-col gap-2">
                            <label htmlFor="" className='text-sm font-semibold text-[#ffffffbe]'>Full name:</label>
                            <input className='bg-[#14191e80] border-b outline-none border-b-[#ffffff30] h-[50px] w-full px-[15px] rounded-[5px]' type="text" name="fullname" id="fullname" placeholder='Aremu Taofeek' required />
                        </div>
                        <div className="inputs flex flex-col gap-2">
                        ...
                        <div className="inputs flex flex-col gap-2">
                            <label htmlFor="" className='text-sm font-semibold text-[#ffffffbe]'>Confirm Password:</label>
                            <input className='bg-[#14191e80] border-b outline-none border-b-[#ffffff30] h-[50px] w-full px-[15px] rounded-[5px]' type="password" name="confirmPassword" id="confirmPassword" minLength={8} placeholder='********' required />
                        </div>
                    </div>
                    <Link href="" className='text-right text-xs text-[#ffffff80]'>Forget Password?</Link>
                    <button className='btn flex items-center gap-2 w-full' type="submit">Create Account <FaArrowRightLong /></button>
                </form>
            </div>
            <div className="md:block hidden img bg-[url(/images/signup.jpeg)] bg-cover bg-right-top bg-no-repeat w-full h-[90vh] rounded-[20px]">
                {/* <Image className='w-full h-[90vh] object-cover object-top' src="/images/signup.jpeg" width={100} height={100} alt='login_picture' /> */}
            </div>
        </div>
